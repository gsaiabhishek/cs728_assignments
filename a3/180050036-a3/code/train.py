...
class Trainer:
	...
	def create_model(self):
		if self.cfg.model.type == 'FFNModel':
			...
			model = FFNModel_Sigmoid(self.cfg.model.input_dim, self.cfg.model.hidden_dim_1, \
				self.cfg.model.hidden_dim_2, self.cfg.model.output_dim)
		elif self.cfg.model.type == 'BertCased':
			model = BertCased(self.cfg.model.bert_dim, self.cfg.model.output_dim, self.cfg.train_args.device,\
	...
	def parse_words(self, parse_type, data, outputs):
		if parse_type == 0:
			words_and_scores = []
			...
			for i in range(len(data)):
				words_and_scores.append([data[i], outputs[i]])
		return words_and_scores

	def evaluate(self, parse_type, dataloader_type, sigmoid_threshold = 0.5, test_index = 0, have_targets = True):
		...
		with torch.no_grad():
			for _, (data, inputs, targets) in enumerate(dataloader):
				if type(inputs) is dict:
					for key in inputs:
						inputs[key] = inputs[key].to(self.cfg.train_args.device)
				else:
				...
				if sigmoid_threshold >= 0:
					words_and_scores.extend(self.parse_words(parse_type, data, outputs.view(-1).detach().cpu().numpy()))
				else:
					words_and_scores.extend(self.parse_words(parse_type, data, ((outputs.max(1))[1]).view(-1).detach().cpu().numpy() ) )
		...
		for epoch in range(self.cfg.train_args.num_epochs):
			...
			if (epoch % self.cfg.train_args.print_every == 0) or (epoch == self.cfg.train_args.num_epochs - 1):
				...
				if ('trn_loss' in print_args) or ('trn_acc' in print_args):
					trn_loss, trn_correct, trn_total, trn_acc, _ = self.evaluate(self.cfg.train_args.parse_type, 'trn', sigmoid_threshold = self.cfg.dataset.sigmoid_threshold)
				...
				if val_set is not None and (('val_loss' in print_args) or ('val_acc' in print_args)):
					val_loss, val_correct, val_total, val_acc, _ = self.evaluate(self.cfg.train_args.parse_type, 'val', sigmoid_threshold = self.cfg.dataset.sigmoid_threshold, 
				...
				if test_sets is not None and (('test_loss' in print_args) or ('test_acc' in print_args)):
					for test_dataloader_index in range(len(self.test_dataloaders)):
						test_loss, test_correct, test_total, test_acc, _ = self.evaluate(self.cfg.train_args.parse_type, 'test', sigmoid_threshold = self.cfg.dataset.sigmoid_threshold, 
